...
const AdminGallery = () => {
  ...
  const fetchPhotos = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/photos`, {
        headers: {
  ...
  const handleDelete = (id: any) => {
    ...
    if (confirmDelete) {
      fetch(`${import.meta.env.VITE_BASE_URL}/api/photos/${id}`, {
        ...
        .then((response) => {
          if (response.ok) {
            setPhotos((prevPhotos) => prevPhotos.filter((photo: any) => photo.id !== id));
            console.log('Photo entry deleted successfully');
          } else {
            console.error('Failed to delete photo entry');
          }
        })
        .catch((error) => {
          console.error('Error deleting photo entry:', error.message);
        });
    }
  };
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 py-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
      <Breadcrumb pageName="Galleri Foto" />
      <div className="flex justify-between items-center mb-4">
        <Link to={'/galeri-foto/tambah'}
        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {photos.map((photo: any, index: number) => (
              <tr key={photo.id}>
                <td className="border-b border-[#b9b9b9] text-center py-4 px-4 dark:border-strokedark">
                  {index + 1}
                </td>
