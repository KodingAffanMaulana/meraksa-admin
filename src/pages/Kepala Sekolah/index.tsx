...
const KepalaSekolah = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/kepala-sekolah`);
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <Breadcrumb pageName="Kepala Sekolah" />
      <Link to="/kepala-sekolah/tambah" className='rounded-lg border text-white py-2 px-3 bg-green-400 hover:bg-green-500'>Tambah Data</Link>
          ...
          <tbody>
            {data.map((item: any, i: number) => (
              <tr key={item.id}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark text-center">
                  <img src={item.image} alt="Kepala Sekolah Image" className="w-16 h-16 object-cover mx-auto" />
                </td>
