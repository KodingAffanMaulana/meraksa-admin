...
const SiswaTable = () => {
  ...
  const fetchData = async (query = '') => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/siswa?page=${currentPage}&nama=${query}`);
      ...
      if (result.status === 200) {
        ...
        setSearchStatus(
          result.data.length > 0
            ? `Ditampilkan ${result.data.length} nama`
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <Breadcrumb pageName="Direktori Siswa" />

      <div className="flex flex-col-reverse md:flex-row gap-2 justify-between items-end md:items-center my-4">
        <Link to="/siswa/tambah" className='rounded-lg border text-white py-2 px-3 bg-green-400 hover:bg-green-500'>Tambah Data</Link>
          ...
          <tbody>
            {data.map((item: any, i: number) => (
              <tr key={item.id}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark text-center">
                  <img src={item.image} alt={item.nama} className="w-16 h-16 object-cover mx-auto" />
                </td>
