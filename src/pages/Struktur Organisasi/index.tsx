...
const StrukturOrganisasi = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/struktur-organisasi`);
  ...
  const handleDelete = async (id: any) => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/struktur-organisasi/${id}`, {
          method: 'DELETE',
          headers: {
          ...
          <tbody>
            {data.map((item: any, i: number) => (
              <tr key={item.id}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark text-center">
                  <img src={item.image} alt={item.nama} className="w-10 h-10 object-cover rounded-full mx-auto" />
                </td>
