...
const SilabusTable = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/silabus`);
          ...
          <tbody>
            {data.map((item: any, i: number) => (
              <tr key={item.id}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <a href={item.file} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
                    Download File
                  </a>
                </td>
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark ">
