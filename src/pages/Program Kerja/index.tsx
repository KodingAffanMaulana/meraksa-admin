...
const ProgramKerjaTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/program-kerja`);
  ...
  const handleDelete = async (id: any) => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/program-kerja/${id}`, {
          method: 'DELETE',
          headers: {
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <Breadcrumb pageName="Program Kerja" />
      <Link to="/progja/tambah" className='rounded-lg border text-white py-2 px-3 bg-green-400 hover:bg-green-500'>Tambah Program Kerja</Link>
