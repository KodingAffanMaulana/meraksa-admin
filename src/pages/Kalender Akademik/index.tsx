...
const KalenderAkademikTable = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/kalender-akademik`);
  ...
  const handleSort = (order: any) => {
    ...
    const sortedData = [...data].sort((a: any, b: any) => {
      return order === 'asc' ? a.tahun.localeCompare(b.tahun) : b.tahun.localeCompare(a.tahun);
    });
    setData(sortedData);
  };

  const handleEdit = (id: any) => {
    navigate(`/kalender/${id}`);
  };
          ...
          <tbody>
            {data.map((item: any, i: number) => (
              <tr key={item.id}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <img src={item.image} alt={`Kalender ${item.tahun}`} className="w-32 mx-auto" />
                </td>
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark ">
...
export default KalenderAkademikTable;
